...
# def simple_color_alignment_loss(pred_images, target_images):
#     """
#     最简单有效的颜色对齐损失 - 只比较RGB通道均值
#     """
#     # 确保输入为float32
...
#             pred_mean = pred_images[i, c].mean()
#             target_mean = target_images[i, c].mean()
#             total_loss += F.mse_loss(pred_mean, target_mean)
    
#     return total_loss / (batch_size * 3)

def latent_alignment_loss(pred_latents, target_latents):
    """
    潜在空间对齐损失 - 直接在latent space进行对齐
    """
    # 确保输入为float32
    pred_latents = pred_latents.float()
    ...
    for i in range(batch_size):
        # 对每个通道计算全局均值和标准差
        for c in range(pred_latents.shape[1]):  # 潜在空间通道数（通常是4）
            pred_mean = pred_latents[i, c].mean()
            target_mean = target_latents[i, c].mean()
            pred_std = pred_latents[i, c].std()
            target_std = target_latents[i, c].std()
            
            # 均值对齐
            total_loss += F.mse_loss(pred_mean, target_mean)
            # 标准差对齐（可选）
            total_loss += F.mse_loss(pred_std, target_std) * 0.5  # 降低权重
    
    return total_loss / (batch_size * pred_latents.shape[1])

def denoised_latent_alignment_loss(model_pred, noisy_latents, target_latents, noise, timesteps, noise_scheduler):
    """
    基于去噪后的潜在表示对齐损失
    """
    # 计算预测的干净潜在表示
    if noise_scheduler.config.prediction_type == "epsilon":
        ...
        pred_clean_latents = (noisy_latents - sqrt_one_minus_alpha_t * model_pred) / sqrt_alpha_t
    elif noise_scheduler.config.prediction_type == "v_prediction":
        pred_clean_latents = noise_scheduler.get_original_sample(noisy_latents, model_pred, timesteps)
    else:
        raise ValueError(f"Unknown prediction type {noise_scheduler.config.prediction_type}")
    
    # 在潜在空间进行对齐
    return latent_alignment_loss(pred_clean_latents, target_latents)
...
def main(args):
    ...
    # Initialize TensorBoard writer AFTER accelerator is created
    tb_writer = None
    if accelerator.is_main_process:
        tensorboard_dir = Path(args.output_dir, "tensorboard_logs")
    ...
    use_color_loss = True
    base_color_loss_weight = 0.1  # 降低权重，避免干扰主要训练
    color_loss_weight = base_color_loss_weight  # 使用固定权重，不要动态调整
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                # if use_color_loss and global_step % 20 == 0:  # 每5步计算一次
                #     try:
                #         # 使用调度器获取预测的干净图像
                #         if noise_scheduler.config.prediction_type == "epsilon":
                #             alpha_t = noise_scheduler.alphas_cumprod[timesteps]
                ...
                #         # 解码到图像空间 - 确保数据类型与VAE权重匹配
                #         with torch.no_grad():
                #             # 使用与VAE相同的数据类型进行解码
                #             pred_clean_latents_scaled = (1 / vae.config.scaling_factor * pred_clean_latents).to(dtype=weight_dtype)
                #             pred_images = vae.decode(pred_clean_latents_scaled).sample
                        
                #         # 将解码后的图像转换为float32进行损失计算
                #         pred_images = pred_images.float()
                #         target_images = batch["pixel_values"].float()
                        
                #         # 使用最简单的颜色对齐损失
                #         color_loss = simple_color_alignment_loss(pred_images, target_images)
                        
                #         # 确保数据类型匹配
                #         if diffusion_loss.dtype != color_loss.dtype:
                #             color_loss = color_loss.to(diffusion_loss.dtype)
                        
                #         total_loss = diffusion_loss + color_loss_weight * color_loss
                        
                #         # 记录颜色损失
                #         if global_step % 100 == 0:
                #             accelerator.log({
                #                 "color_loss": color_loss.float().item(),
                #                 "diffusion_loss": diffusion_loss.float().item()
                #             }, step=global_step)
                    
                #     except Exception as e:
                #         logger.warning(f"Color loss computation failed: {e}")
                #         total_loss = diffusion_loss
                ...
                # 添加潜在空间对齐损失
                total_loss = diffusion_loss
                if use_color_loss and global_step % 10 == 0:  # 每10步计算一次
                    try:
                        # 方法1: 直接对齐去噪后的潜在表示
                        latent_loss = denoised_latent_alignment_loss(
                            model_pred, noisy_latents, latents, noise, timesteps, noise_scheduler
                        )
                        
                        # 方法2: 额外的噪声残差对齐（修复设备问题）
                        # 计算真实的噪声残差
                        true_noise_residual = noisy_latents - latents
                        
                        # 计算预测的噪声残差 - 修复设备不匹配问题
                        if noise_scheduler.config.prediction_type == "epsilon":
                            # 确保betas在正确的设备上
                            betas = noise_scheduler.betas.to(timesteps.device)
                            pred_noise_residual = model_pred * betas[timesteps].view(-1, 1, 1, 1).sqrt()
                        else:
                            # 对于v_prediction，需要转换
                            alpha_t = noise_scheduler.alphas_cumprod[timesteps]
                            sqrt_alpha_t = torch.sqrt(alpha_t).view(-1, 1, 1, 1)
                            sqrt_one_minus_alpha_t = torch.sqrt(1 - alpha_t).view(-1, 1, 1, 1)
                            pred_noise_residual = sqrt_one_minus_alpha_t * model_pred
                        
                        # 噪声残差对齐损失
                        noise_residual_loss = latent_alignment_loss(pred_noise_residual, true_noise_residual)
                        
                        # 组合损失
                        combined_latent_loss = latent_loss + 0.3 * noise_residual_loss
                        
                        # 确保数据类型匹配
                        if diffusion_loss.dtype != combined_latent_loss.dtype:
                            combined_latent_loss = combined_latent_loss.to(diffusion_loss.dtype)
                        
                        total_loss = diffusion_loss + color_loss_weight * combined_latent_loss
                        
                        # 记录损失到TensorBoard和accelerator
                        if global_step % 100 == 0:
                            loss_dict = {
                                "latent_alignment_loss": latent_loss.float().item(),
                                "noise_residual_loss": noise_residual_loss.float().item(),
                                "combined_latent_loss": combined_latent_loss.float().item(),
                                "diffusion_loss": diffusion_loss.float().item(),
                        ...
                        logger.warning(f"Latent alignment loss computation failed: {e}")
                        total_loss = diffusion_loss
            ...
            # 更新进度条显示详细损失信息
            logs = {
                "total_loss": total_loss.detach().item() if 'total_loss' in locals() else diffusion_loss.detach().item(),
                "diff_loss": diffusion_loss.detach().item(),
                "lr": lr_scheduler.get_last_lr()[0]
            }
            
            # 如果有潜在空间损失，也显示
            if 'combined_latent_loss' in locals():
                logs["latent_loss"] = combined_latent_loss.detach().item()
            
            progress_bar.set_postfix(**logs)
            
            # 每步都记录基础损失到accelerator
            basic_logs = {"loss": logs["total_loss"], "lr": logs["lr"]}
            accelerator.log(basic_logs, step=global_step)
